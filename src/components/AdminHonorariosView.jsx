...
import GeneradorManualPrestaciones from './GeneradorManualPrestaciones';

const AdminHonorariosView = ({ userData }) => {
  const [reportsStatus, setReportsStatus] = React.useState([]);
  const [loading, setLoading] = React.useState(true);
  const [activeTab, setActiveTab] = React.useState('consolidado'); // 'consolidado' | 'manual'
  const [selectedFuncIdForManual, setSelectedFuncIdForManual] = React.useState('');
  ...
  // Filtrar los informes según el término de búsqueda
  const filteredReports = reportsStatus.filter(row => 
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto space-y-8 animate-fade-in bg-[#F8FAFC]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-2">
        <div>
          <h1 className="text-3xl font-black text-secondary tracking-tight">Honorarios e Informes</h1>
      ...
      {activeTab === 'consolidated' ? (
          ...
          <div className="lg:col-span-3 space-y-4">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4">
              <div className="relative flex-1">
                ...
                <input 
                  type="text" 
                  placeholder="Buscar por funcionario o RUT..." 
                  value={searchTerm}
                  onChange={e => setSearchTerm(e.target.value)}
                  className="w-full bg-tertiary border-none rounded-xl py-3 pl-12 pr-4 text-sm font-medium focus:ring-2 focus:ring-primary/20" 
                />
              </div>
                      ...
                      <tr key={row.id} className="hover:bg-gray-50 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-2">
                            <button 
                              onClick={() => {
                                setSelectedFuncIdForManual(row.id);
                                setActiveTab('manual');
                              }}
                              className="p-2 text-gray-400 hover:text-primary transition-colors" 
                              title="Llenar Informe Manual / Desglose"
                            >
                              <Eye size={18} />
                            </button>
                            <button 
        ...
        <GeneradorManualPrestaciones 
          userData={userData} 
          selectedFuncIdProp={selectedFuncIdForManual}
          onClearFuncIdProp={() => setSelectedFuncIdForManual('')}
        />
      )}
    </div>
  );
